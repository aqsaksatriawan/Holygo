...
import { ChevronLeft, Key, ChevronRight, LogOut, User } from "lucide-react";
...
const SettingsPage: React.FC = () => {
  ...
  useEffect(() => {
    const storedUser = localStorage.getItem("holygo_user");
  ...
  const handleLogout = () => {
    const confirmLogout = window.confirm("Apakah Anda yakin ingin Sign Out?");

    if (confirmLogout) {
      localStorage.removeItem("holygo_user");
      router.push("/login");
    }
  };
    ...
    <div className="max-w-md mx-auto bg-[#FDFBFA] min-h-screen flex flex-col font-sans text-[#3D4759] shadow-2xl border-x border-gray-100">
      ...
      <main className="flex-1 p-6 space-y-8">
        ...
        <div className="bg-white p-4 rounded-[28px] shadow-sm flex items-center gap-4 border border-gray-50">
          <div className="w-16 h-16 rounded-2xl bg-[#EEF2FF] shrink-0 border-2 border-white shadow-sm flex items-center justify-center">
            <User className="w-8 h-8 text-[#51309E]" strokeWidth={2.5} />
          </div>
